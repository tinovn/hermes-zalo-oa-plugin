"""Bộ lọc nội dung đi ra (``outbound_scrub``) — chép từ plugin `zalo-personal`.

Cùng bộ ca kiểm thử, chỉ đổi cách nạp: bên kia phải exec phần đầu adapter.py
để né ``gateway.*``, ở đây scrub đã tách hẳn thành module riêng nên import
thẳng. Giữ nguyên các ca để hai plugin còn đối chiếu được với nhau.
"""
...
import outbound_scrub as _NS_MOD  # noqa: E402

_NS = vars(_NS_MOD)
_scrub_outgoing = _NS_MOD.scrub_outgoing
...
class TestClassifierCallSites(unittest.TestCase):
    ...
    def test_every_call_site_passes_is_owner(self):
        ...
        self.assertGreaterEqual(len(calls), 1, "call sites disappeared — update this test")
        for args in calls:
...
class TestBrandRedaction(unittest.TestCase):
    ...
    def test_no_deployment_specific_rewrites(self):
        # Plugin này dùng chung: không nhét thay-tên-thương-hiệu của riêng ai.
        out = _scrub_outgoing("Dạ anh vào HostBill để xem hóa đơn nha.")
        self.assertIn("HostBill", out)
